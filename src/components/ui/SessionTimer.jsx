...
const SessionTimer = ({ isActive, startTime, style, isPaused, pauseStartTime, accumulatedTime = 0 }) => {
    ...
    return (
        <div style={style}>
            <div className={`text-white font-bold rounded-lg sm:rounded-xl shadow-lg px-2 sm:px-3 md:px-4 py-2 sm:py-3 md:py-4 flex items-center min-w-[150px] sm:min-w-[180px] md:min-w-[200px] min-h-[40px] sm:min-h-[50px] md:min-h-[60px] ${!isActive && totalTimeSeconds === 0 ? 'bg-gray-400' : isPaused ? 'bg-gray-500' : 'bg-blue-500'}`}>
