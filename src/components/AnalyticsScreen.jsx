    ...
    <div style={dashboardStyle} className="text-white">
      <div className="w-full px-4 py-4 md:px-6 md:py-6">
        <div className="flex items-center justify-between gap-3 mb-5">
          <button
            className="rounded-xl border border-white/20 bg-slate-500/35 hover:bg-slate-400/40 text-white px-4 py-2.5 flex items-center gap-2 font-bold"
            onClick={() => navigate(-1)}
          >
            <FaArrowLeft />
            <span>Back</span>
          </button>
          <div className="text-4xl md:text-5xl font-black tracking-tight">Analytics</div>
          <button
            type="button"
            onClick={handleExportCsv}
            className="rounded-xl border border-white/20 bg-slate-500/35 hover:bg-slate-400/40 text-white px-4 py-2.5 font-extrabold"
          >
            Export CSV
          </button>
        </div>
        ...
        <div className="rounded-3xl border border-white/10 bg-white/10 overflow-hidden mb-4 shadow-[0_15px_40px_rgba(0,0,0,0.35)]">
          ...
          <div className="bg-black/25 border-t border-white/10 px-4 md:px-5 py-3 flex items-center justify-between gap-3">
            <div className="inline-flex items-center gap-2 text-2xl font-bold">
              <FaLayerGroup className="text-white/70 text-lg" />
              <span>{levelDisplay}</span>
            </div>
            <div className="text-2xl font-black tracking-wide">
              <span className="text-white/75 text-lg font-semibold mr-1">PIN:</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
          <div className="rounded-2xl border border-white/10 bg-white/10 p-4">
            ...
            <div className="text-4xl font-black leading-none">{summary?.overall?.totalAttempts ?? 0}</div>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/10 p-4">
            <div className="flex items-center gap-2 text-white/80 text-sm font-semibold mb-1">
              <FaCheckCircle />
            ...
            <div className="text-4xl font-black leading-none">{summary?.overall?.totalCorrect ?? 0}</div>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/10 p-4">
            <div className="flex items-center gap-2 text-white/80 text-sm font-semibold mb-1">
              <FaChartLine />
            ...
            <div className="text-4xl font-black leading-none">{pct(summary?.overall?.accuracy ?? 0)}</div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/10 p-4 mb-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <div className="text-xs text-white/70 mb-1 font-semibold">Level:</div>
              <select
                className="w-full rounded-xl border border-white/10 bg-slate-700/80 px-3 py-2.5 text-white"
                value={level}
            ...
            <div>
              <div className="text-xs text-white/70 mb-1 font-semibold">Operation:</div>
              <select
                className="w-full rounded-xl border border-white/10 bg-slate-700/80 px-3 py-2.5 text-white"
                value={operation}
        ...
        <div className="rounded-2xl border border-white/10 bg-white/10 p-4 mb-5">
          ...
          {Array.isArray(struggling?.strugglingFacts) && struggling.strugglingFacts.length === 0 && (
            <div className="text-white/70">No struggling facts yet.</div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {struggling?.strugglingFacts?.map((sf, idx) => (
              <button
                key={idx}
                className="text-left bg-black/20 hover:bg-black/30 border border-white/10 rounded-xl p-3 transition flex items-start justify-between gap-3"
                onClick={() => openFact({ operation: sf.fact.operation, a: sf.fact.a, b: sf.fact.b })}
              >
                <div>
                  <div className="font-semibold text-2xl leading-tight">{sf.question}</div>
                  <div className="text-sm text-white/75">
                    attempts: {sf.totalAttempts} - accuracy {pct(sf.accuracy)}
                  </div>
                </div>
                <span className="rounded-lg border border-white/10 bg-white/10 px-2.5 py-1 text-lg font-bold">
                  {sf.totalAttempts}
                </span>
              </button>
            ))}
        ...
        <div className="rounded-2xl border border-white/10 bg-white/10 p-4">
          ...
          <div className="flex items-center justify-between mt-4 text-sm text-white/70">
            ...
            <div className="flex gap-2">
              <button
                className="rounded-xl border border-white/20 bg-white/85 hover:bg-white text-black px-4 py-2 font-semibold disabled:opacity-50"
